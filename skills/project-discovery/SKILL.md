...
- **Architecture diagram** — If missing or clearly outdated, create an overall system/component diagram (e.g. mermaid). Prefer committed paths such as `docs/architecture.md`. See [diagram-conventions.md](references/diagram-conventions.md) for format and placement.
- **Flow diagrams** — For important flows (e.g. request path, deployment, data flow) when useful and not already documented. Place under `docs/` (e.g. `docs/flows/request-flow.md`).
...
- For ticket-related artifacts, write to `docs/jira/<TICKET>/`. For general project docs, write to `docs/` or the appropriate committed path. For drafts the user may not want to commit, output in chat or to a temporary path and suggest where to move.
