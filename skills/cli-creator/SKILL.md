...
The resulting command must run on Windows, Linux, and macOS unless the user explicitly asks for an OS-specific tool. Prefer one portable command surface over separate per-OS behavior. When wrappers are useful, keep them thin and make the underlying implementation portable.
...
6. Smoke test from another repo or a temporary directory, not only with `cargo run` or package-manager wrappers. Run the OS-appropriate command lookup (`command -v`, `where.exe`, or `Get-Command`), `<tool-name> --help`, and `<tool-name> --json doctor`.
...
After the CLI works, create or update a small skill for it. Use `$skill-creator` when it is available. Prefer the user's personal skill home for private companion skills (for example `$CODEX_HOME/skills/<tool-name>/SKILL.md` for Codex) unless the user explicitly names a repository skill path (such as a repo-local `.codex/skills/...` directory) or another skill repo.
...
- Windows, Linux, and macOS setup/run notes when path, quoting, or install commands differ.
